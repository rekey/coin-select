...
jobs:
  select-pairs:
    ...
    steps:
      ...
      - name: 结果变化则提交并推送
        run: |
          if [ ! -f pairlist.json ] || [ ! -f selection_report.csv ] || \
             ! diff -q out/pairlist.json pairlist.json >/dev/null 2>&1 || \
             ! diff -q out/selection_report.csv selection_report.csv >/dev/null 2>&1; then
